...
cache_create_impl(char *name,		/* name of the cache */
...
{
  byte_t *p_io = vp;
  md_addr_t tag = CACHE_TAG(cache, addr);
  md_addr_t set = CACHE_SET(cache, addr);
  md_addr_t block_offset = CACHE_BLK(cache, addr);
  struct cache_blk_t *block, *replace;
  int latency = 0;
  ...
  /* FIXME:
     ((addr + (nbytes - 1)) > ((addr & ~cache->blk_mask) + (cache->bsize - 1))) */
  if ((addr + nbytes) > ((addr & ~cache->blk_mask) + cache->bsize))
    fatal("cache: access error: access spans block, addr 0x%08x", addr);

  /* permissions are checked on cache misses */

  /* check for a fast hit: access to same block */
  if (CACHE_TAGSET(cache, addr) == cache->last_tagset) {
    /* hit in the same block */
    block = cache->last_block;
    goto cache_fast_hit;
  }
    
  if (cache->hsize) {
    /* higly-associativity cache, access through the per-set hash tables */
    const int hindex = CACHE_HASH(cache, tag);

    for (block = cache->sets[set].hash[hindex]; block; block=block->hash_next) {
	  if (block->tag == tag && (block->status & CACHE_BLK_VALID))
	    goto cache_hit;
	}
  } else {
    /* low-associativity cache, linear search the way list */
    for (block = cache->sets[set].way_head; block; block=block->way_next) {
      if (block->tag == tag && (block->status & CACHE_BLK_VALID))
        goto cache_hit;
    }
  }
  ...
  switch (cache->policy) {
    case LRU:
    case FIFO:
      replace = cache->sets[set].way_tail;
      update_way_list(&cache->sets[set], replace, Head);
      break;
    case Random: {
      int bindex = myrand() & (cache->assoc - 1);
      replace = CACHE_BINDEX(cache, cache->sets[set].blks, bindex);
      break;
    }
    default:
  ...
  if (cache->hsize)
    unlink_htab_ent(cache, &cache->sets[set], replace);
  ...
  if (replace->status & CACHE_BLK_VALID) {
    cache->replacements++;

    if (repl_addr)
      *repl_addr = CACHE_MK_BADDR(cache, replace->tag, set);

    /* don't replace the block until outstanding misses are satisfied */
    latency += BOUND_POS(replace->ready - now);

    /* stall until the bus to next level of memory is available */
    latency += BOUND_POS(cache->bus_free - (now + latency));

    /* track bus resource usage */
    cache->bus_free = MAX(cache->bus_free, (now + latency)) + 1;

    if (replace->status & CACHE_BLK_DIRTY) {
	  /* write back the cache block */
	  cache->writebacks++;
      latency += cache->blk_access_fn(Write,
                                      CACHE_MK_BADDR(cache, replace->tag, set),
                                      cache->bsize, replace, now+latency);
	}
  }

  /* update block tags */
  replace->tag = tag;
  replace->status = CACHE_BLK_VALID;	/* dirty bit set on update */

  /* read data block */
  latency += cache->blk_access_fn(Read, CACHE_BADDR(cache, addr), cache->bsize, replace, now + latency);

  /* copy data out of cache block */
  if (cache->balloc) {
    CACHE_BCOPY(cmd, replace, block_offset, p_io, nbytes);
  }

  /* update dirty status */
  if (cmd == Write)
    replace->status |= CACHE_BLK_DIRTY;

  /* get user block data, if requested and it exists */
  if (udata)
    *udata = replace->user_data;

  /* update block status */
  replace->ready = now+latency;

  /* link this entry back into the hash table */
  if (cache->hsize)
    link_htab_ent(cache, &cache->sets[set], replace);
 ...
 cache_hit: /* slow hit handler */
  ...
  if (cache->balloc) {
    CACHE_BCOPY(cmd, block, block_offset, p_io, nbytes);
  }

  /* update dirty status */
  if (cmd == Write)
    block->status |= CACHE_BLK_DIRTY;

  /* if LRU replacement and this is not the first element of list, reorder */
  if (block->way_prev && cache->policy == LRU) {
    /* move this block to head of the way (MRU) list */
    update_way_list(&cache->sets[set], block, Head);
  }

  /* tag is unchanged, so hash links (if they exist) are still valid */

  /* record the last block to hit */
  cache->last_tagset = CACHE_TAGSET(cache, addr);
  cache->last_block = block;

  /* get user block data, if requested and it exists */
  if (udata)
    *udata = block->user_data;

  /* return first cycle data is available to access */
  return (int) MAX(cache->hit_latency, (block->ready - now));
